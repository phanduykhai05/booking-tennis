  ...
  return (
    <div aria-modal="true" className="fixed inset-0 z-[1200] flex items-end justify-center" role="dialog">
      ...
      <section className={`relative flex w-full max-w-[430px] flex-col overflow-hidden bg-white shadow-[0_-12px_32px_rgba(0,36,22,.34)] transition-[height,border-radius] duration-300 ease-out ${isExpanded ? "h-[100dvh] rounded-none" : "h-[480px] rounded-t-[22px]"} ${styles.sheet}`}>
        ...
        <div className="relative -mt-3 rounded-t-[18px] bg-white px-5 pb-5 pt-7"><span className="absolute -top-5 left-1/2 flex -translate-x-1/2 items-center gap-1 rounded-full bg-[#21c58d] px-4 py-2 text-[14px] font-bold text-white"><Star aria-hidden="true" fill="currentColor" size={19} />{ratingLabel}</span><div className="flex gap-3"><span className="flex size-12 shrink-0 items-center justify-center rounded-full bg-[#f7f2d8] text-xs font-black text-[#00713d] ring-1 ring-[#d9ca8d]">{content.previewVenueMark}</span><div className="min-w-0"><h2 className="truncate text-[17px] font-bold text-[#064b30]">{venue.name}</h2><div className="mt-1 flex flex-wrap gap-1">{categories.map((category) => <span className="rounded-full border border-[#39a6ef] px-2 py-0.5 text-[11px] text-[#1689d2]" key={category}>{category}</span>)}</div></div></div><div className="mt-4 space-y-2 text-[14px] text-[#075038]"><p className="flex items-center gap-3"><MapPin aria-hidden="true" className="text-[#008447]" size={18} fill="currentColor" />{venue.address}</p><p className="flex items-center gap-3"><Clock3 aria-hidden="true" className="text-[#008447]" size={18} fill="currentColor" />{venue.openingLabel}</p>{phone && <p className="flex items-center gap-3"><Phone aria-hidden="true" className="text-[#008447]" size={18} fill="currentColor" />{phone}</p>}</div></div>
        ...
        <VenuePreviewTabContent activeTab={activeTab} address={venue.address} openingLabel={venue.openingLabel} preview={detail?.preview ?? null} venue={venue} />
        </div>
      </section>
    </div>
  );
