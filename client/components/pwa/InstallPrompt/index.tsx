  ...
  return (
    <>
      ...
      >
        ...
        <button className="shrink-0 rounded-xl bg-[#0f9b58] px-3 py-2 text-sm font-bold text-white" onClick={install} type="button">Cài</button>
        <button aria-label="Đóng thông báo" className="shrink-0 text-xl leading-none text-[#68716d]" onClick={closePrompt} type="button">×</button>
      </aside>
      ...
      {isInstructionsOpen && (
        <div aria-modal="true" className="fixed inset-0 z-[10001] flex items-end bg-black/50 p-4 sm:items-center sm:justify-center" role="dialog">
          <div className="w-full max-w-md rounded-2xl bg-white p-5 text-[#173524] shadow-2xl">
            ...
            <button className="mt-5 w-full rounded-xl bg-[#0f9b58] px-4 py-2.5 text-sm font-bold text-white" onClick={closePrompt} type="button">Đã hiểu</button>
          </div>
        </div>
      )}
    </>
